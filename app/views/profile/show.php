...
.ie-photo-item.primary::after { content: 'ראשית'; position: absolute; bottom: 0; left: 0; right: 0; background: rgba(242,208,13,0.9); color: #0f0e08; font-size: 10px; font-weight: 800; text-align: center; padding: 2px; }
...
<div class="admin-bar" id="adminBar">
    <div class="flex items-center gap-3">
        ...
        <span class="text-primary font-bold text-sm">מצב עריכה</span>
    </div>
    <div class="flex items-center gap-3">
        <button id="toggleEditBtn" onclick="toggleEditMode()" class="flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-bold transition-all bg-primary/10 text-primary border border-primary/30 hover:bg-primary/20">
            ...
            <span id="toggleEditLabel">הפעל עריכה</span>
        </button>
...
<section class="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-20">
    ...
    <div class="lg:col-span-7 relative group overflow-hidden rounded-3xl luxury-shadow gallery-item editable-section" style="height: 750px;" onclick="openLightbox(0)">
        ...
        <div class="edit-overlay" onclick="event.stopPropagation(); openEditor('photos')">
            <button class="edit-btn-float"><span class="material-symbols-outlined">photo_library</span> ערוך תמונות</button>
        </div>
        <?php endif; ?>
    </div>
    ...
    <div class="lg:col-span-5 flex flex-col justify-between">
        <div>
            ...
            <div class="flex items-center gap-4 mb-10 pb-10 border-b border-white/5 editable-section relative">
                ...
                <div>
                    <p class="text-primary font-black uppercase tracking-[0.3em] text-[10px] mb-1">מקום מגורים</p>
                    <span id="location" class="text-2xl font-light text-white"></span>
                </div>
                <?php if (!empty($isAdmin)): ?>
                <div class="edit-overlay" onclick="openEditor('basic')">
                    <button class="edit-btn-float"><span class="material-symbols-outlined">edit</span> פרטים בסיסיים</button>
                </div>
                <?php endif; ?>
            </div>
            ...
            <div class="space-y-6 mb-10 editable-section relative">
                <div class="flex items-center gap-4 p-4 bg-card/50 rounded-xl border border-white/5">
                    ...
                    <div><p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest">גיל</p>
                ...
                <div class="flex items-center gap-4 p-4 bg-card/50 rounded-xl border border-white/5">
                    ...
                    <div><p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest">מקצוע</p>
                    <p id="detOccupation" class="text-lg text-white font-medium"></p></div>
                </div>
                <div class="flex items-center gap-4 p-4 bg-card/50 rounded-xl border border-white/5">
                    <span class="material-symbols-outlined text-primary text-2xl">school</span>
                    <div><p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest">השכלה</p>
                ...
                <div class="flex items-center gap-4 p-4 bg-card/50 rounded-xl border border-white/5">
                    ...
                    <div><p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest">שפות</p>
                ...
                <div class="flex items-center gap-4 p-4 bg-card/50 rounded-xl border border-white/5">
                    ...
                    <div><p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest">תחביבים</p>
                    <p id="detHobbies" class="text-lg text-white font-medium"></p></div>
                </div>
                <?php if (!empty($isAdmin)): ?>
                <div class="edit-overlay" onclick="openEditor('details')">
                    <button class="edit-btn-float"><span class="material-symbols-outlined">edit</span> פרטים נוספים</button>
                </div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="space-y-4">
            <a id="profileMsgLink" href="#" onclick="handleMsgClick(event)" class="w-full py-5 rounded-2xl font-black text-lg uppercase tracking-widest hover:brightness-110 transition-all shadow-2xl flex items-center justify-center gap-3" style="background:linear-gradient(135deg,#f2d00d 0%,#b89b06 100%);color:#12110a;text-decoration:none;">
                ...
                <span id="profileMsgBtn" <?php if (!empty($isAdmin)): ?>onclick="profileInlineEdit(this,'profile_msg_btn');event.preventDefault();event.stopPropagation();"<?php endif; ?>>שלח הודעה</span>
            </a>
            <div class="grid grid-cols-2 gap-4">
                <a id="profileVideoLink" href="#" onclick="handleVideoClick(event)" class="border-2 border-primary/40 text-primary py-4 rounded-2xl font-black text-sm uppercase tracking-wider hover:bg-primary/10 transition-all flex items-center justify-center gap-2 text-decoration-none">
                    ...
                    <span id="profileVideoBtn" <?php if (!empty($isAdmin)): ?>onclick="profileInlineEdit(this,'profile_video_btn');event.preventDefault();event.stopPropagation();"<?php endif; ?>>שיחת וידאו</span>
                </a>
                <a id="profileGiftLink" href="#" onclick="handleGiftClick(event)" class="bg-card text-white py-4 rounded-2xl font-black text-sm uppercase tracking-wider hover:bg-card/80 transition-all flex items-center justify-center gap-2 border border-white/10 text-decoration-none">
                    ...
                    <span id="profileGiftBtn" <?php if (!empty($isAdmin)): ?>onclick="profileInlineEdit(this,'profile_gift_btn');event.preventDefault();event.stopPropagation();"<?php endif; ?>>שלח מתנה</span>
                </a>
            </div>
        </div>
    </div>
...
<section class="mb-20 editable-section relative">
    <div class="flex items-end justify-between mb-8 border-b border-white/10 pb-4">
        <h3 class="text-4xl font-bold text-white">גלריה</h3>
...
<section id="videosSection" class="mb-20 editable-section relative" style="display:none;">
    <div class="flex items-end justify-between mb-8 border-b border-white/10 pb-4">
        <h3 class="text-4xl font-bold text-white">סרטונים</h3>
    ...
    <div class="edit-overlay" onclick="openEditor('videos')">
        <button class="edit-btn-float"><span class="material-symbols-outlined">videocam</span> ערוך סרטונים</button>
    </div>
    <?php endif; ?>
</section>
...
<section class="grid grid-cols-1 lg:grid-cols-3 gap-16 py-20 border-t border-white/10">
    <div class="lg:col-span-2 editable-section relative">
        <h4 class="text-4xl font-bold mb-8 text-white">במילים שלה</h4>
        <div class="space-y-8">
        ...
        <div class="edit-overlay" onclick="openEditor('bio')">
            <button class="edit-btn-float"><span class="material-symbols-outlined">edit</span> ערוך ביוגרפיה</button>
        </div>
        <?php endif; ?>
    </div>
    <div class="bg-card/50 p-10 rounded-3xl border border-primary/10 shadow-2xl h-fit editable-section relative">
        <h5 class="text-primary font-black uppercase tracking-[0.3em] text-[11px] mb-10 flex items-center gap-3">
            <span class="material-symbols-outlined text-xl">tune</span> העדפות התאמה
        </h5>
        <ul class="space-y-8">
            <li class="flex items-start gap-4">
                <span class="material-symbols-outlined text-primary text-2xl mt-0.5">check_circle</span>
                <div><p class="font-black text-white uppercase text-[10px] tracking-widest mb-1">מטרת הקשר</p>
                <p id="prefGoal" class="text-slate-300 text-lg font-light"></p></div>
            </li>
            <li class="flex items-start gap-4">
                <span class="material-symbols-outlined text-primary text-2xl mt-0.5">check_circle</span>
                <div><p class="font-black text-white uppercase text-[10px] tracking-widest mb-1">מצב משפחתי</p>
                <p id="prefMarital" class="text-slate-300 text-lg font-light"></p></div>
            </li>
            <li class="flex items-start gap-4">
                <span class="material-symbols-outlined text-primary text-2xl mt-0.5">check_circle</span>
                <div><p class="font-black text-white uppercase text-[10px] tracking-widest mb-1">מזל</p>
                <p id="prefZodiac" class="text-slate-300 text-lg font-light"></p></div>
            </li>
        </ul>
        <?php if (!empty($isAdmin)): ?>
        <div class="edit-overlay" onclick="openEditor('preferences')">
            <button class="edit-btn-float"><span class="material-symbols-outlined">edit</span> ערוך העדפות</button>
        </div>
        <?php endif; ?>
    </div>
</section>
...
<div class="relative w-full max-w-[500px] mx-4 bg-card p-8 rounded-xl border border-white/10 shadow-2xl">
    ...
    <div class="text-center mb-6">
        ...
        <h3 class="text-xl font-bold text-white">שלח הודעה ל-<span id="msgProfileName" class="text-primary"></span></h3>
    </div>
    <div id="msgLoginPrompt" class="hidden text-center py-6">
        <p class="text-slate-400 mb-4">עליך להתחבר כדי לשלוח הודעות</p>
        <a id="msgLoginLink" href="#" class="px-8 py-3 rounded-lg font-bold hover:opacity-90 transition-all inline-block" style="background:#f2d00d;color:#12110a;">התחבר עכשיו</a>
    </div>
    <form id="msgForm" class="space-y-4">
        <textarea id="msgContent" rows="5" class="w-full bg-background-dark border border-white/10 rounded-lg py-3 px-4 text-white focus:border-primary outline-none resize-none" placeholder="כתוב את ההודעה שלך כאן..." required></textarea>
        <div id="msgError" class="hidden text-center text-sm font-bold text-red-400"></div>
        <div id="msgSuccess" class="hidden text-center text-sm font-bold text-green-400">ההודעה נשלחה בהצלחה!</div>
        <button type="submit" id="msgSubmitBtn" class="w-full py-3 rounded-lg font-bold hover:brightness-110 transition-all" style="background:linear-gradient(135deg,#f2d00d 0%,#b89b06 100%);color:#12110a;">שלח הודעה</button>
    </form>
</div>
</div>
...
const maritalMap = { single: 'רווקה', divorced: 'גרושה', widowed: 'אלמנה' };
...
async function loadProfile() {
    try {
        ...
        if (!res.ok) {
            document.getElementById('loading').innerHTML = '<p class="text-red-400 text-xl">פרופיל לא נמצא</p>';
            return;
        }
        ...
        // Details
        document.getElementById('detAge').textContent = `בת ${p.age}`;
        ...
        // Preferences
        document.getElementById('prefGoal').textContent = 'נישואין רציניים';
        document.getElementById('prefMarital').textContent = maritalMap[p.marital_status] || 'רווקה';
        ...
        document.getElementById('photoCount').textContent = `${allPhotoUrls.length} תמונות`;
        ...
        if (allPhotoUrls.length === 0) {
            gallery.innerHTML = '<p class="text-slate-500 col-span-full text-center py-12">אין תמונות עדיין</p>';
        }
        ...
        if (videos.length > 0 || isAdmin) {
            ...
            document.getElementById('videoCount').textContent = `${videos.length} סרטונים`;
            if (videos.length > 0) {
                ...
                videoGallery.innerHTML = '<p class="text-slate-500 col-span-full text-center py-12">אין סרטונים עדיין</p>';
            }
        }

        document.getElementById('loading').style.display = 'none';
    } catch (err) {
        document.getElementById('loading').innerHTML = '<p class="text-red-400 text-xl">שגיאה בטעינת הפרופיל</p>';
    }
}
...
function openMessageModal() {
    ...
    document.getElementById('msgSubmitBtn').textContent = 'שלח הודעה';
    if (!user) {
...
document.getElementById('msgForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (res.ok) {
            ...
            document.getElementById('msgContent').value = '';
            document.getElementById('msgSubmitBtn').textContent = 'נשלח!';
...
function toggleEditMode() {
    ...
    document.getElementById('toggleEditLabel').textContent = active ? 'כבה עריכה' : 'הפעל עריכה';
...
function profileInlineEdit(el, key) {
    ...
    popup.innerHTML = `
        <div style="font-size:15px;font-weight:700;color:#f2d00d;margin-bottom:14px;display:flex;align-items:center;gap:6px;">
            <span class="material-symbols-outlined" style="font-size:18px;">edit</span> עריכת כפתור
        </div>
        <div style="margin-bottom:12px;">
            <label style="font-size:12px;color:rgba(255,255,255,0.5);margin-bottom:4px;display:block;">טקסט הכפתור</label>
        ...
        <div style="margin-bottom:14px;">
            <label style="font-size:12px;color:rgba(255,255,255,0.5);margin-bottom:4px;display:block;">קישור (וואצאפ / URL חיצוני / ריק = הודעה במערכת)</label>
            ...
            <div style="margin-top:6px;display:flex;gap:6px;flex-wrap:wrap;">
                ...
                <button type="button" class="tel-preset" style="background:rgba(242,208,13,0.1);color:#f2d00d;border:1px solid rgba(242,208,13,0.2);border-radius:6px;padding:4px 10px;font-size:11px;font-weight:700;cursor:pointer;">טלפון</button>
                <button type="button" class="clear-preset" style="background:rgba(255,255,255,0.05);color:rgba(255,255,255,0.4);border:1px solid rgba(255,255,255,0.1);border-radius:6px;padding:4px 10px;font-size:11px;cursor:pointer;">נקה (הודעה במערכת)</button>
            </div>
        </div>
        <div style="display:flex;gap:8px;justify-content:flex-start;">
            <button class="save-btn" style="background:#f2d00d;color:#0f0e08;border:none;border-radius:8px;padding:10px 24px;font-size:14px;font-weight:700;cursor:pointer;">שמור</button>
            <button class="cancel-btn" style="background:transparent;color:rgba(255,255,255,0.6);border:1px solid rgba(255,255,255,0.15);border-radius:8px;padding:10px 20px;font-size:13px;cursor:pointer;">ביטול</button>
        </div>`;
...
function openEditor(section) {
    ...
    switch(section) {
        case 'basic':
            title.textContent = 'עריכת פרטים בסיסיים';
            body.innerHTML = `
                <div class="grid grid-cols-2 gap-4">
                    <div class="ie-field"><label>שם</label><input id="ie_name" value="${esc(p.name)}"/></div>
                    <div class="ie-field"><label>גיל</label><input id="ie_age" type="number" value="${p.age || ''}"/></div>
                    <div class="ie-field"><label>עיר</label><input id="ie_city" value="${esc(p.city)}"/></div>
                    <div class="ie-field"><label>מדינה</label>
                        <select id="ie_country">
                            <option value="moldova" ${p.country==='moldova'?'selected':''}>מולדובה</option>
                            <option value="ukraine" ${p.country==='ukraine'?'selected':''}>אוקראינה</option>
                        </select>
                    </div>
                    <div class="ie-field"><label>מצב משפחתי</label>
                        <select id="ie_marital">
                            <option value="single" ${p.marital_status==='single'?'selected':''}>רווקה</option>
                            <option value="divorced" ${p.marital_status==='divorced'?'selected':''}>גרושה</option>
                            <option value="widowed" ${p.marital_status==='widowed'?'selected':''}>אלמנה</option>
                        </select>
                    </div>
                    <div class="ie-field"><label>ילדים</label><input id="ie_children" value="${esc(p.children)}"/></div>
                </div>
                <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-white/10">
                    <button class="ie-cancel-btn" onclick="closeEditor()">ביטול</button>
                    <button class="ie-save-btn" onclick="saveBasic()">שמור שינויים</button>
                </div>`;
            break;

        case 'details':
            title.textContent = 'עריכת פרטים נוספים';
            body.innerHTML = `
                <div class="grid grid-cols-2 gap-4">
                    <div class="ie-field"><label>עיסוק</label><input id="ie_occupation" value="${esc(p.occupation)}"/></div>
                    <div class="ie-field"><label>השכלה</label><input id="ie_education" value="${esc(p.education)}"/></div>
                    <div class="ie-field"><label>שפות</label><input id="ie_languages" value="${esc(p.languages)}"/></div>
                    <div class="ie-field"><label>תחביבים</label><input id="ie_hobbies" value="${esc(p.hobbies)}"/></div>
                    <div class="ie-field"><label>גובה</label><input id="ie_height" value="${esc(p.height)}"/></div>
                    <div class="ie-field"><label>משקל</label><input id="ie_weight" value="${esc(p.weight)}"/></div>
                </div>
                <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-white/10">
                    <button class="ie-cancel-btn" onclick="closeEditor()">ביטול</button>
                    <button class="ie-save-btn" onclick="saveDetails()">שמור שינויים</button>
                </div>`;
            break;

        case 'bio':
            title.textContent = 'עריכת ביוגרפיה';
            body.innerHTML = `
                <div class="ie-field"><label>מה היא מחפשת</label><input id="ie_looking_for" value="${esc(p.looking_for)}"/></div>
                <div class="ie-field"><label>אודותיה</label><textarea id="ie_about" rows="6">${esc(p.about)}</textarea></div>
                <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-white/10">
                    <button class="ie-cancel-btn" onclick="closeEditor()">ביטול</button>
                    <button class="ie-save-btn" onclick="saveBio()">שמור שינויים</button>
                </div>`;
            break;

        case 'preferences':
            title.textContent = 'עריכת העדפות';
            body.innerHTML = `
                <div class="ie-field"><label>מחפשת</label><input id="ie_pref_looking" value="${esc(p.looking_for)}"/></div>
                <div class="ie-field"><label>מצב משפחתי</label>
                ...
                <div class="ie-field"><label>מזל</label><input id="ie_pref_zodiac" value="${esc(p.zodiac)}" placeholder="מזל טלה, שור, תאומים..."/></div>
                <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-white/10">
                    <button class="ie-cancel-btn" onclick="closeEditor()">ביטול</button>
                    <button class="ie-save-btn" onclick="savePreferences()">שמור שינויים</button>
                </div>`;
            break;

        case 'photos':
            title.textContent = 'עריכת תמונות';
            const photos = p.photos || [];
            body.innerHTML = `
                <div class="ie-field"><label>תמונות נוכחיות (${photos.length})</label></div>
                    ...
                    ${photos.map(ph => `
                        <div class="ie-photo-item ${ph.is_primary == 1 ? 'primary' : ''}" data-id="${ph.id}">
                            ...
                            <div style="position:absolute;bottom:0;left:0;right:0;display:flex;gap:2px;">
                                ${ph.is_primary != 1 ? `<button onclick="ieSetPrimary(${ph.id})" style="flex:1;background:rgba(242,208,13,0.8);color:#0f0e08;border:none;font-size:9px;padding:3px;cursor:pointer;font-weight:700;" title="הגדר כתמונה ראשית">★</button>` : ''}
                            </div>
                ...
                <div class="ie-field mt-4">
                    ...
                    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
                        ...
                        <span id="ie_upload_progress" class="text-sm text-primary hidden">מעלה...</span>
                    </div>
                </div>
                <div id="ie_upload_error" class="text-sm text-red-400 mt-2 hidden"></div>
                <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-white/10">
                    <button class="ie-cancel-btn" onclick="closeEditor()">סגור</button>
                </div>`;
            break;

        case 'videos':
            title.textContent = 'עריכת סרטונים';
            const videos = p.videos || [];
            body.innerHTML = `
                <div class="ie-field"><label>סרטונים נוכחיים (${videos.length})</label></div>
                    ...
                    ${videos.map(v => `
                        <div class="ie-video-item" data-id="${v.id}" style="position:relative;background:#0f0e08;border:1px solid rgba(255,255,255,0.1);border-radius:12px;overflow:hidden;padding:12px;display:flex;gap:12px;align-items:center;">
                            ...
                            <div style="flex:1;min-width:0;">
                                <p style="color:white;font-size:13px;font-weight:700;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${v.title || 'ללא כותרת'}</p>
                            ...
                            <button onclick="ieDeleteVideo(${v.id})" style="background:rgba(220,38,38,0.8);color:white;border:none;border-radius:8px;width:32px;height:32px;display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;" title="מחק">
                ...
                <div class="ie-field mt-4">
                    ...
                    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
                        ...
                        <span id="ie_video_progress" class="text-sm text-primary hidden">מעלה...</span>
                    </div>
                </div>
                <div class="ie-field mt-2">
                    <label>או הוסף קישור לסרטון (YouTube / URL)</label>
                    <div style="display:flex;gap:8px;">
                        ...
                        <button onclick="ieAddVideoUrl()" style="background:rgba(242,208,13,0.9);color:#0f0e08;border:none;border-radius:10px;padding:10px 16px;font-size:13px;font-weight:800;cursor:pointer;">הוסף</button>
                    </div>
                </div>
...
async function saveBio() {
    try {
        ...
        showToast('הביוגרפיה עודכנה בהצלחה');
        closeEditor();
        loadProfile();
    } catch { showToast('שגיאה בשמירה', 'error'); }
}
...
async function savePreferences() {
    try {
        ...
        showToast('ההעדפות עודכנו בהצלחה');
        closeEditor();
        loadProfile();
    } catch { showToast('שגיאה בשמירה', 'error'); }
}
...
async function ieUploadPhotos(input) {
    ...
    for (let i = 0; i < files.length; i++) {
        prog.textContent = `מעלה ${i + 1} מתוך ${files.length}...`;
        ...
        try {
                ...
                if (ph) {
                    grid.innerHTML += `
                        <div class="ie-photo-item" data-id="${ph.id}">
                            <img src="${ph.photo_url}"/>
                            <div style="position:absolute;bottom:0;left:0;right:0;display:flex;gap:2px;">
                                <button onclick="ieSetPrimary(${ph.id})" style="flex:1;background:rgba(242,208,13,0.8);color:#0f0e08;border:none;font-size:9px;padding:3px;cursor:pointer;font-weight:700;" title="הגדר כתמונה ראשית">★</button>
                            </div>
                            <button class="ie-photo-del" onclick="ieDeletePhoto(${ph.id})">&times;</button>
                        </div>`;
                    successCount++;
            ...
            errorMessages.push(`${files[i].name}: שגיאת רשת`);
        }
    }

    if (successCount > 0) {
        prog.textContent = `${successCount} תמונות הועלו בהצלחה!`;
        showToast(`${successCount} תמונות הועלו בהצלחה`);
    } else {
        prog.textContent = 'לא הועלו תמונות';
    }
...
async function ieUploadVideo(input) {
    ...
    if (file.size > 100 * 1024 * 1024) {
        errEl.textContent = 'גודל הסרטון חורג מ-100MB';
        errEl.classList.remove('hidden');
        prog.classList.add('hidden');
        return;
    }

    prog.textContent = 'מעלה סרטון...';
    try {
        ...
        const upData = await upRes.json();
        if (!upRes.ok) {
            errEl.textContent = upData.error || 'שגיאה בהעלאה';
            errEl.classList.remove('hidden');
            prog.classList.add('hidden');
            return;
        }
        ...
        if (url) {
            ...
            if (!videoRes.ok) {
                ...
                errEl.textContent = d.error || 'שגיאה בשמירה';
                errEl.classList.remove('hidden');
                prog.classList.add('hidden');
                return;
            }
            showToast('הסרטון הועלה בהצלחה!');
            prog.textContent = 'הסרטון הועלה!';
            const res = await fetch(BASE + '/api/profiles/' + profileId);
            currentProfile = await res.json();
            loadProfile();
            openEditor('videos');
