...
$pageDescription = 'גלו את סיפורי ההצלחה של זוגות שמצאו אהבת אמת דרך שירות השידוכים היוקרתי שלנו';
$currentPage = 'stories';
...
<section class="py-16 md:py-24 px-6 text-center">
    <div class="max-w-4xl mx-auto">
        <h2 id="storiesHeroTitle" class="text-4xl md:text-6xl font-black mb-6 gold-gradient-text">סיפורי הצלחה שמתחילים כאן</h2>
        <p id="storiesHeroSubtitle" class="text-xl text-slate-400 font-light max-w-2xl mx-auto">
            אנחנו גאים להיות חלק מהרגעים המרגשים ביותר בחיים. גלו את הזוגות שמצאו אהבת אמת מעבר לים והקימו בית חם בישראל.
        </p>
    </div>
</section>
...
<section class="px-6 md:px-20 pb-20">
    <div id="storiesGrid" class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <p class="text-slate-500 text-center col-span-full py-12">טוען סיפורים...</p>
    </div>
</section>
...
async function loadStories() {
    ...
    try {
        ...
        if (!stories.length) {
            grid.innerHTML = '<p class="text-slate-500 text-center col-span-full py-12">אין סיפורי הצלחה עדיין</p>';
            return;
        }
        ...
        grid.innerHTML = '<p class="text-red-400 text-center col-span-full py-12">שגיאה בטעינת סיפורים</p>';
    }
}
loadStories();

// Load stories page settings
...
<section class="bg-surface py-20 px-6 border-y border-border-gold">
    <div class="max-w-4xl mx-auto text-center">
        <h2 id="storiesCtaTitle" class="text-3xl md:text-5xl font-extrabold text-slate-100 mb-8 leading-tight">התחילו את סיפור ההצלחה שלכם</h2>
        <p id="storiesCtaSubtitle" class="text-lg text-slate-400 mb-10">
            הכלה המושלמת מחכה לך במרחק קליק אחד. הצטרפו למאות גברים שכבר מצאו את האושר שלהם דרכנו.
        </p>
...
<div class="relative w-full max-w-[550px] mx-4 bg-[#1c1a0e] border border-primary/30 p-8 rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto" dir="rtl">
    ...
    <h3 class="text-xl font-bold text-primary mb-6" id="storyModalTitle">הוספת סיפור הצלחה</h3>
    <form id="storyEditForm" class="space-y-4">
        <input type="hidden" id="storyEditId"/>
        <div class="space-y-1">
            <label class="text-sm text-slate-300 font-bold">שמות הזוג</label>
            <input id="storyEditNames" class="w-full bg-white/5 border border-white/15 rounded-lg px-4 py-3 text-white" placeholder="דוד ואולגה"/>
        </div>
        <div class="space-y-1">
            <label class="text-sm text-slate-300 font-bold">כותרת</label>
            <input id="storyEditTitle" class="w-full bg-white/5 border border-white/15 rounded-lg px-4 py-3 text-white" placeholder="כותרת הסיפור"/>
        </div>
        <div class="space-y-1">
            <label class="text-sm text-slate-300 font-bold">הסיפור</label>
            <textarea id="storyEditText" class="w-full bg-white/5 border border-white/15 rounded-lg px-4 py-3 text-white resize-none" rows="4" placeholder="תכתבו כאן את הסיפור..."></textarea>
        </div>
        <div class="space-y-1">
            <label class="text-sm text-slate-300 font-bold">תמונה</label>
            <div class="flex gap-3 items-center">
                <input id="storyEditImage" class="flex-1 bg-white/5 border border-white/15 rounded-lg px-4 py-3 text-white text-sm" placeholder="URL או העלה תמונה"/>
                <button type="button" onclick="uploadStoryImage()" class="bg-primary/20 text-primary px-4 py-3 rounded-lg font-bold text-sm hover:bg-primary/30">העלה</button>
            </div>
            <input type="file" id="storyImageFile" accept="image/*" class="hidden"/>
        </div>
        <div class="space-y-1">
            <label class="text-sm text-slate-300 font-bold">תאריך חתונה</label>
            <input id="storyEditDate" type="date" class="w-full bg-white/5 border border-white/15 rounded-lg px-4 py-3 text-white"/>
        </div>
        <div class="flex gap-3 pt-2">
            <button type="submit" class="flex-1 bg-primary text-background-dark py-3 rounded-xl font-bold text-lg">שמור</button>
            <button type="button" onclick="closeStoryModal()" class="px-6 bg-white/10 text-slate-300 py-3 rounded-xl font-bold">ביטול</button>
        </div>
    </form>
</div>
</div>
...
<script>
// Override story rendering to add edit/delete buttons in edit mode
...
loadStories = async function() {
    ...
    try {
        ...
        addCard.innerHTML = '<div class="text-center"><span class="material-symbols-outlined text-primary text-5xl mb-3">add_circle</span><p class="text-primary font-bold text-lg">הוסף סיפור חדש</p></div>';
...
function openStoryModal(story = null) {
    ...
    document.getElementById('storyModalTitle').textContent = story ? 'עריכת סיפור' : 'הוספת סיפור הצלחה';
...
async function deleteStory(id) {
    ...
    try {
        ...
        if (res.ok) { aieToast('סיפור נמחק'); loadStories(); }
        else { aieToast('שגיאה במחיקה', 'error'); }
    } catch(e) { aieToast('שגיאה', 'error'); }
}
...
window.uploadStoryImage = function() {
    ...
    fileInput.onchange = async function() {
        ...
        try {
            ...
            else aieToast(data.error || 'שגיאה', 'error');
        } catch(e) { aieToast('שגיאה בהעלאה', 'error'); }
    };
    fileInput.click();
};
...
document.getElementById('storyEditForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (res.ok) {
            aieToast(id ? 'סיפור עודכן!' : 'סיפור נוסף!');
            closeStoryModal();
            await fetchStories();
            loadStories();
...
// Load stories with edit buttons
