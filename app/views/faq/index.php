...
<section class="relative overflow-hidden py-16 md:py-20 px-6">
    ...
    <div class="max-w-[960px] mx-auto text-center md:text-right relative z-10">
        <h1 class="text-slate-100 text-5xl font-black leading-tight tracking-[-0.033em] font-display mb-4">שאלות נפוצות</h1>
        <p class="text-gold-muted text-lg font-normal leading-normal max-w-2xl">כאן תמצאו תשובות לכל השאלות בנוגע לשירותי השידוכים היוקרתיים שלנו. אנחנו מחויבים לשקיפות מלאה, דיסקרטיות ואיכות ללא פשרות.</p>
    </div>
</section>
...
<div class="max-w-[960px] mx-auto">
    <div id="faqList" class="flex flex-col gap-4">
        <p class="text-gold-muted text-center py-8">טוען שאלות...</p>
    </div>
    ...
    <div class="mt-16 bg-border-gold/20 border border-primary/30 rounded-2xl p-8 @container">
        <div class="flex flex-col @[600px]:flex-row items-center justify-between gap-8">
            <div class="flex flex-col gap-2 text-center @[600px]:text-right">
                <h3 class="text-slate-100 text-2xl font-bold font-display">זקוקים לעזרה או מידע נוסף?</h3>
                <p class="text-gold-muted text-base">המומחים שלנו כאן כדי ללוות אתכם בדרך למציאת זוגיות איכותית ומתגמלת.</p>
            </div>
...
async function loadFaqs() {
    ...
    try {
        ...
        if (!faqs.length) {
            el.innerHTML = '<p class="text-gold-muted text-center py-8">אין שאלות נפוצות עדיין</p>';
            return;
        }
        ...
        el.innerHTML = '<p class="text-red-400 text-center py-8">שגיאה בטעינה</p>';
    }
}
loadFaqs();
</script>
...
<div class="relative w-full max-w-[500px] mx-4 bg-[#1c1a0e] border border-primary/30 p-8 rounded-2xl shadow-2xl" dir="rtl">
    ...
    <h3 class="text-xl font-bold text-primary mb-6" id="faqModalTitle">הוספת שאלה</h3>
    <form id="faqEditForm" class="space-y-4">
        <input type="hidden" id="faqEditId"/>
        <div class="space-y-1">
            <label class="text-sm text-slate-300 font-bold">שאלה</label>
            <input id="faqEditQuestion" class="w-full bg-white/5 border border-white/15 rounded-lg px-4 py-3 text-white" placeholder="השאלה..."/>
        </div>
        <div class="space-y-1">
            <label class="text-sm text-slate-300 font-bold">תשובה</label>
            <textarea id="faqEditAnswer" class="w-full bg-white/5 border border-white/15 rounded-lg px-4 py-3 text-white resize-none" rows="4" placeholder="התשובה..."></textarea>
        </div>
        <div class="space-y-1">
            <label class="text-sm text-slate-300 font-bold">סדר מיון</label>
        ...
        <div class="flex gap-3 pt-2">
            ...
            <button type="button" onclick="closeFaqModal()" class="px-6 bg-white/10 text-slate-300 py-3 rounded-xl font-bold">ביטול</button>
        </div>
    </form>
</div>
</div>
...
loadFaqs = async function() {
    ...
    try {
        ...
        addBtn.innerHTML = '<button onclick="openFaqModal()" class="bg-primary text-background-dark px-8 py-3 rounded-xl font-bold text-lg hover:scale-105 transition-transform">+ הוסף שאלה חדשה</button>';
...
function openFaqModal(faq = null) {
    ...
    document.getElementById('faqModalTitle').textContent = faq ? 'עריכת שאלה' : 'הוספת שאלה';
...
async function deleteFaq(id) {
    ...
    try {
        ...
        if (res.ok) { aieToast('שאלה נמחקה'); loadFaqs(); }
...
document.getElementById('faqEditForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (res.ok) {
            aieToast(id ? 'שאלה עודכנה!' : 'שאלה נוספה!');
            closeFaqModal();
            loadFaqs();
